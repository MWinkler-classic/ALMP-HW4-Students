...
class Experiment:
    ...
    def plan_single_cube_passing(self, cube_i, cubes,
        ...
        right_meeting_point_conf = self.plan_single_arm(planner, cube_conf, self.right_arm_meeting_confs, description, active_arm, "move",
                             left_arm_start, cubes_after_pickup, Gripper.STAY, Gripper.STAY, env)  # gripper_pre: stay closed, gripper_post: stay closed (holding cube)
        ...
        left_meeting_point_conf = self.plan_single_arm(planner, left_arm_start, self.left_arm_meeting_confs, description, active_arm, "move",
                             right_meeting_point_conf, cubes_at_meeting, Gripper.STAY, Gripper.STAY, env)  # gripper_pre: stay open, gripper_post: stay open
        ...
        self.plan_single_arm(planner, self.left_arm_meeting_conf, left_arm_end_conf, description, active_arm, "move",
                             self.right_arm_meeting_confs, cubes_at_placement, Gripper.STAY, Gripper.STAY, env)  # gripper_pre: stay closed, gripper_post: stay closed (holding cube)
    ...
    def plan_experiment(self, DEMO=False):
        ...
        base_meeting_coords = [((1-right_x_bias)*left_arm[0] + right_x_bias*right_arm[0]),
                               ...
                               0.5] # Valid Z value found via simulation search
        # base_meeting_coords = [1.5, 1.5, 0.5]
        ...
        print("meetings coords = ", base_meeting_coords, " left meeting coords = ", left_meeting_coords, " right meeting coords = ", right_meeting_coords)
        ...
        if DEMO:
            visualizer.draw_two_robots(conf_left=self.left_arm_meeting_confs[0], conf_right=self.right_arm_meeting_confs[0])
